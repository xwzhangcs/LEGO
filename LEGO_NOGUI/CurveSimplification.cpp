...
cv::Point3f optimizeByBFGS(const std::vector<cv::Point2d>& points, cv::Point3d init_points) {
	cv::Point3d ans;
...
float compute_angle(cv::Point2d a, cv::Point2d b){
...
float compute_abs_angle(cv::Point2d a, cv::Point2d b){
	double norm_a = cv::norm(a);
	double norm_b = cv::norm(b);
	a.x = a.x / norm_a;
	a.y = a.y / norm_a;
...
float compute_interval(cv::Point2d start, cv::Point2d mid, cv::Point2d end){
...
bool valid_curve(double threshold, const std::vector<cv::Point2d>& points, cv::Point2d center, double radius, cv::Rect bbox){
	bool bValid = true;
	// compute max_error
	double max_error = 0;
	for (int i = 0; i < points.size(); i++){
	...
	int CurveSimplification::approxContour(std::vector<cv::Point2f>& input, util::Polygon &output, float epsilon, float curve_threshold){
		...
		for (int i = 0; i < input.size();){
			...
			do{
				...
				if (valid_curve(threshold, points, center, result.z, bbox)){
					bValid = true;
					next_p = dis;
					dis++;
				}
		...
		//std::cout << "final_contour size  is " << final_contour.size() << std::endl;
		if (bContainCurve == 1){
