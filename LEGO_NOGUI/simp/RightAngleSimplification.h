...
namespace simp {
	...
	public:
		static util::Polygon simplify(const util::Polygon& polygon, int resolution, float orientation, float min_hole_ratio);
		static void decomposePolygon(util::Polygon& polygon);

	private:
		static std::tuple<float, int, int> simplifyContour(const util::Ring& contour, util::Ring& result, int resolution, float orientation);
		...
		static std::vector<cv::Point> RightAngleSimplification::proposedContour(const std::vector<cv::Point>& contour, int x1, int x2, int y1, int y2, int new_x1, int new_x2, int new_y1, int new_y2);
